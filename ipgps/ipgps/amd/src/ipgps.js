...
function($, Ajax, Notification, Str) {
    var newAction, newUserid;
    var changeVisibility = function(action, userid,latitude,longitude) {
        var nAction = oppositeAction(action);
        Ajax.call([{
                    methodname: 'block_ipgps_setUserUbicacion',
                    args: { userid: userid, action: action,latitude:latitude,longitude:longitude },
                    done: function(result){
                        changeVisibilityIconAttr(nAction,result);
                        $("#aLink").attr('data-action',nAction);
                    },
                    fail: Notification.exception,
        }]);
    };
    
    var getGeo = function(action, userid){
    ...
    function onSuccess(position) {
        ...
        changeVisibility(newAction, newUserid,latitude,longitude);
    }
    
    function onError(error) {
        alert('Error: ' + error.message);
        changeVisibility('hide', newUserid,'','');
    }
